...
4. Mongoose to integrate with MongoDB
	...
	export default (): Record<string, any> => ({
		database: {
			...
			name: process.env.DATABASE_NAME || 'parkinson',
...
6. Role and Permission Management.
	...
	- Permission List
		...
		```ts
		// src/modules/permission/permission.constant.ts
...
8. Database Migration with nestjs-command
	...
	import { PermissionList } from 'src/modules/permission/permission.constant';
...
10. Request Validation with Class Validation
	...
	- User Create Validation Schema
		...
		```ts
		// src/modules/user/validation/user.create.validation.ts
...
12. Data Transformer with Class Transformer
	...
	```ts
	// src/modules/user/transformer/user.transformer.ts
...
13. Basic Token Auth with Decorator
	...
	```ts
	// src/app/app.controller.ts
	import { AuthBasicGuard } from 'src/modules/auth/auth.decorator';

...
import {REDIS_CLIENT_PROVIDER, REDIS_TOKEN_PROVIDER} from "../redis/redis.constants";
import {UsersService} from "../users/users.service";
import {AcceptLanguageResolver, I18nModule, QueryResolver} from "nestjs-i18n";
import {PaymentType} from "./orders.constants";
import {LocationNFTStatus, LocationPurchaseStatus, LocationStatus, LocationType} from "../locations/locations.contants";
import {EntityManager, LessThanOrEqual, MoreThanOrEqual, UpdateResult} from "typeorm";
...
describe('Order controller', () => {
    ...
    it('Checkout: buy existed location by cash', async () => {
        mockManager.update = (entity, criteria, updateValue): Promise<UpdateResult> => {
            switch (entity) {
                case Location:
                    expect(criteria).toEqual({id: 1, version: 3})
                    expect(updateValue).toEqual({purchase_status: LocationPurchaseStatus.UNAUTHORIZED, version: 4})
        ...
        const rs = await controller.create(dto, { headers: {}, socket: {}, connection: { remoteAddress: 'localhost' } }, new User() );
        ...
        mockManager.update = (entity, criteria, updateValue): Promise<UpdateResult> => {
            switch (entity) {
                case Location:
                    ...
                    expect(updateValue).toEqual({purchase_status: LocationPurchaseStatus.UNAUTHORIZED, version: 2})

                    return Promise.resolve(updated);
                case LocationHandle:
                    expect(criteria).toEqual({ name: 'my-new-custom-location', total: 2})
        ...
        mockManager.update = (entity, criteria, updateValue): Promise<UpdateResult> => {
            switch (entity) {
                case Location:
                    ...
                    expect(updateValue).toEqual({purchase_status: LocationPurchaseStatus.UNAUTHORIZED, version: 99})
...
const mockManager = {
            ...
            case UserPackage:
                return {
                    save: (item: UserPackage) => {
                        expect(item).toEqual(Object.assign(new UserPackage(), {
                            package_id: 2,
                            user_id: -1,
    ...
    save: (item) => {
        if (item instanceof Location) {
            ...
            expect(item).toEqual(Object.assign(new Location(), {
                lat: 20,
                long: 100,
